...
<body>
	...
	<script>
		...
		async function cargardatos() {
			try {
				//cargar la api
				const respuesta = await fetch("../api/admin/adm_perfil_cargar.php");
				const resultado = await respuesta.json();

				const respuesta2 = await fetch("../api/admin/adm_ver_carreras.php");
		...
		document.getElementById("formactu").addEventListener("submit", async function(e) {
			...
			try {
				const respuesta = await fetch("../api/admin/adm_perfil_actualizar.php", {
					method: "POST",
					body: formulario
				});
		...
		document.getElementById("formcam").addEventListener("submit", async function(e) {
			...
			const formulario = new FormData(this);
			try {
				const respuesta = await fetch("../api/admin/adm_perfil_camcont.php",{
					method: "POST",
					body: formulario
				});
		...
		document.getElementById("formeli").addEventListener("submit", async function(e) {
			e.preventDefault;
			try {
				const respuesta = await fetch("../api/admin/adm_perfil_eliminar.php",{
					method: "POST"
				});

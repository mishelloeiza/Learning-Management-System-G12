<?php
    ...
    if(strlen($contrasena_new) < 8){
        Response::error("La nueva contraseña debe tener al menos 8 caracteres", -1003, 400);
    }

    try {
        $cn = new Connection("tutor");

        $stmt = $cn->prepare("SELECT contrasena FROM usuarios_2 WHERE id_usuario = ?");
        $stmt->execute([$id]);
        $usuario = $stmt->fetch(PDO::FETCH_ASSOC);

        if($usuario === false || !password_verify($contrasena_actu, $usuario['contrasena'])){
            Response::error("La contraseña actual es incorrecta", -1004, 400);
        }

        $cifrada = password_hash($contrasena_new, PASSWORD_DEFAULT);

        $stmt = $cn->prepare("UPDATE usuarios_2 SET contrasena = ? WHERE id_usuario = ?");
